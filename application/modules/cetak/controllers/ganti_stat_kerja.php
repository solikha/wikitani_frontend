...
class ganti_stat_kerja extends Print_Controller {
        ...
        if (isset($_GET['lyn_id'])){
           ...
           $data['data_pencetakan'] = array("kota" => $kotakbri, "tanggal_cetak" => "2005-05-05");
            if ($position == "kiri") {
                ...
                $html = $this->load->view("v_ganti_stat_kerja_kiri", $data, true);
            } elseif ($position == "kanan") {
                $html = $this->load->view("v_ganti_stat_kerja_kanan", $data, true);
            } else {
			...
			$pdfFilePath = "Amandemen Status Kerja ".getArrayDef($datalayanan,'pemohon').".pdf";
