...
class ganti_lapor_diri extends Print_Controller {
    ...
    public function cetak($position) {
        ...
        if (isset($_GET['lyn_id'])){
            ...
            $nama_kbri = $datakbri['nama'];
            $datalayanan = $this->mcetakalamat->loadWniLayanan($lyn_id);
            //$this->load->helper('url');
            //print_r($datalayanan); die();
            $data['datalayanan'] = $datalayanan;
           $data['data_pencetakan'] = array("nama_kbri" => $nama_kbri, "tanggal_cetak" => "2005-05-05");
            if ($position == "kiri") {
                //peringatan parameter $data,true wajib dilampirkan
                $html = $this->load->view("v_ganti_lapor_diri_kiri", $data, true);
            } elseif ($position == "kanan") {
                $html = $this->load->view("v_ganti_lapor_diri_kanan", $data, true);
            } else {
            ...
            $pdfFilePath = "Amandemen Lapor Diri ".getArrayDef($datalayanan,'pemohon').".pdf";
            $pdf->WriteHTML($html);
