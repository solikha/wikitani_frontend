...
class Wftest extends MY_Controller {
    ...
    public function index(){
        $this->load->model('m_workflow', 'mflow');
        $baseurl = $this->mflow->getBaseUrl() . "workflow/wftest/";
        $params = array(
            ...
            "urledit" => $baseurl . "edit",
            "urldelete" => $baseurl . "delete"
        );

        $this->load->view('perizinan/imp_jalan/v_lampiran_new', $params);
    }
    ...
    public function edit(){
        ...
        try {
            $lampiranid = isset($_POST['lampiranid']) ? $_POST['lampiranid'] : 0;
            //print_r($_POST);
            $sql = "
                SELECT * 
                FROM app_lampiran l 
                    LEFT JOIN app_lampiran_jenis j ON l.jenisdokumen = j.jenislampiranid 
                WHERE lampiranid = :lampiranid";
    ...
    public function docsimpan(){
        ...
        try {
            if (isset($_POST))
            {
                ...
                $uploaddir = BASEPATH . '../uploads/' . $form['workflowid'] . '/';
                ...
                // INSERT QUERY
                $sql = "INSERT INTO app_lampiran ( workflowid, namadokumen, jenisdokumen, detail, format, lokasi, status, filename, filetype, jenisfile ) 
    ...
    public function download($lampiranid) {
        $sql = "SELECT * FROM app_lampiran WHERE lampiranid = :lampiranid";
        $params = array("lampiranid"=>$lampiranid);
        $res = $this->mdb->QueryData('default', $sql, $params, 'record');
        ...
        if (count($res) > 0) {
            ...
            if (!empty($file['filename'])) {
                $filepath = BASEPATH . '../uploads/' . $file['workflowid'] . '/' . $file['filename'];
        ...
        if (count($res) > 0) {
            ...
            try {
                if (!empty($lampiran['filename'])) {
                    $filepath = BASEPATH . '../uploads/' . $lampiran['workflowid'] . '/' . $lampiran['filename'];
                    unlink($filepath);
                }

                $sql = "DELETE FROM app_lampiran WHERE lampiranid = :lampiranid";
                $params = array("lampiranid"=>$lampiranid);
