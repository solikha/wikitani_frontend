...
<style>
    ...
    .led-offline{
        width: 30px;
        background: url(<?php echo $basedir.'images/pin/'; ?>led48-offline.png) no-repeat center;
        background-size: 24px 24px;
    }
    .led-live{
        width: 30px;
        background: url(<?php echo $basedir.'images/pin/'; ?>led48-live.png) no-repeat center;
        background-size: 24px 24px;
    }
    .led-warning{
        width: 30px;
        background: url(<?php echo $basedir.'images/pin/'; ?>led48-warning.png) no-repeat center;
        background-size: 24px 24px;
    }
    .led-danger{
        width: 30px;
        background: url(<?php echo $basedir.'images/pin/'; ?>led48-danger.png) no-repeat center;
        background-size: 24px 24px;
    }
...
<!--            <a href="#" class="btn btn-success btn-sm btn-round" style="margin-left:12px; margin-top: 0px; margin-bottom: 2px"
               onclick="send_disarm_button({{id}})" >
                <i class="ace-icon fa fa-unlock-alt  bigger-150"></i>
            </a>
            <a href="#" class="btn btn-warning btn-sm btn-round" style="margin-left:12px; margin-top: 0px; margin-bottom: 4px"
        ...
        <!-- /section:pages/timeline.info -->
        <div class="widget-box transparent collapsed" style="margin-left:40px;min-height:130px">
            <div class="widget-header" style=" padding-right:6px;">
                <div class="widget-toolbar" >
                    <a id="status-live-{{id}}" class="" style="height: 30px; display:inline-block; " href="#">&nbsp;</a>
                    <a href="#" data-action="collapse">
                        <i class="ace-icon fa fa-chevron-down"></i>
                    </a>
                </div>                
                <h4 class="jobsite-id-{{id}}-title" style="margin-bottom:-10px">{{nama}} </h4> &nbsp;
...
//                updatePopup(vmarker);
        ...
        var updateStatusData = function(){
            console.log("updateStatusData");
            for(var vidx = 0; vidx<dataJobsite.length; vidx++){
                var data = dataJobsite[vidx];
                console.log(data);
                var vtime = data['last_upd_zona'];
                vtime = vtime.replace("+07", "");
                var vwaktu = new Date(vtime);
                var vnow = new Date();
                var vwaktu2 = vnow.getTime()-vwaktu.getTime();
                var vminutes = vwaktu2/(1000*60);
                var vstatus = 'offline';
                if(vminutes<1){
                    vstatus = 'live';
                } else if (vminutes<60){
                    vstatus = 'warning';
                } else if (vminutes<(24*60)){
                    vstatus = 'danger';
                }
                //console.log(vtime);
                //vstatus = 'live';
                var vid = data['jobsite_id'];
                $('#status-live-'+vid).removeClass();
                $('#status-live-'+vid).addClass('led-'+vstatus);
                console.log(vwaktu);
                console.log(vwaktu2);
                //console.log(vhours);
            }
